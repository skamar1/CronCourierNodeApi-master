...
router.get('/', async (req, res) => {
    ...
    var queryStr = "select users.AdminLevel, companies.companyId,companies.companyName,companies.address,companies.city,companies.zip,companies.phone,companies.email,companies.DefaultCourierId,companies.smsSender,companies.smsApiKey,CourierList.Username,CourierList.Password,CompanyCode,CompanyPassword,BillingCode,BillingCodeCy,companies.userID,companies.courierCustomerCode, companies.companyName + case when ISNULL(companies.courierCustomerCode,'') <> '' then ' - ' else '' END + ISNULL(companies.courierCustomerCode,'') [CompanyNameWithCourierCode],ws.name [Group],CourierList.id,CourierList.code,CourierList.name,CourierList.ApiKey from companies left join users on users.userid = companies.userID left join users as ws on ws.userid = users.wholesalerid LEFT JOIN CourierList on CourierList.id = companies.DefaultCourierId " + options + " Order by courierCustomerCode";
